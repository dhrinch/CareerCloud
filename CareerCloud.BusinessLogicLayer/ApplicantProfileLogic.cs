        ...
        protected override void Verify(ApplicantProfilePoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                if (poco.CurrentSalary < 0)
                    exceptions.Add(new ValidationException(111, $"CurrentSalary in {poco} ({poco.CurrentSalary}) cannot be negative"));                

                if (poco.CurrentRate < 0)
                    exceptions.Add(new ValidationException(112, $"CurrentRate in {poco} ({poco.CurrentRate}) cannot be negative"));                
            }
